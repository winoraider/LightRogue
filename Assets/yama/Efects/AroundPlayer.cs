...
public class AroundPlayer : MonoBehaviour
{
    ...
    // ZŽ²‰ñ“]
    Vector3 RotateAroundZ(Vector3 original_position, float angle, float radius)
    {
        Vector3 v = original_position;
        //v.y = radius;
        float a = angle * Mathf.Deg2Rad;
        float x = Mathf.Cos(a) * radius; // * v.x - Mathf.Sin(a) * v.y;
        float y = Mathf.Sin(a) * radius; // * v.x + Mathf.Cos(a) * v.y;
        float z = v.z;
